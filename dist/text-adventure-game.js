(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["Text Adventure Game Engine"] = factory();
	else
		root["Text Adventure Game Engine"] = factory();
})(this, function() {
...
var Editor = function Editor(json_data) {
    this.json_data = json_data;
    // holds game engine
    this.game = null;
    // initializes empty new game engine
    this.new_game = function () {
        this.game = new Engine();return this;
    };
    ...
    this.factory = { location: new LocationFactory()
        ...
        return new EngineSerializer().to_json(this.game);
    };
    // loads game configuration from json file
    this.from_json = function () {
        this.game = new EngineSerializer().from_json(this.json_data);return this;
    };
    // returns game engine object ready to be played
    this.start_game = function () {
        this.game.see();return this.game;
    };
    // adds location to the game engine
    this.add_location = function (l) {
        this.game.locations.add(l);return this;
    };
    // sets player location
    this.set_player_location = function (location_id) {
        this.game.player.location_id = location_id;return this;
    };
    // adds event listener to game engine
    this.add_listener = function (event_name, event_handler) {
        this.game.events.subscribe(event_name, event_handler);return this;
    };
...
var ActionSelector = function ActionSelector(editor, event_type, condition) {
    this.editor = editor;
...
var EngineSerializer = function EngineSerializer() {
    ...
    this.condition_parser["LocationCondition"] = function (c) {
        return new LocationCondition(c.location_id);
    };

    // todo: refactor to action_factory so there is single method to instantiate action
    // parses model from json into Action model
    this.action_parser = {};
    this.action_parser["SetLocationStateAction"] = function (a) {
        return new SetLocationStateAction(a.location_id, a.new_state, a.repeat);
    };
    ...
    this.from_json = function (json_data) {
        ...
        for (var key in model.locations) {
            ...
            // set single values
            var lf = editor.factory.location["new"]().id(key).set_state(location_settings.state);

            lf.set_criteria(location_settings.criteria);
            // set texts for states
            for (var state in location_settings.texts) {
                lf.set_text(state, location_settings.texts[state]);
            }
...
var Engine = function Engine(gameJSON) {
    var _this = this;
    ...
    this.events.event_published_callback = function (e, data) {
        data.engine.event_handlers.execute(e, data.engine);
    };
    // get current location
    this.location = function () {
        return this.locations.get(this.player.location_id);
    };
    ...
    this.evaluate_criteria = function (evalfunc, interaction, criteria) {
        for (var key in criteria) {
            var success = evalfunc(interaction, criteria[key]);
            if (success) {
                return key;
            }
        }
        // modify the state key
        _this.location().state = 'non-response';
        return false;
    };

    // Evaluate text input
    this.interact = function (interaction) {
        var loc = _this.locations.get(_this.player.location_id);
        // This just a demo eval function to check for keywords in a list
        var evalfunc = function evalfunc(interaction, keywordList) {
            if (keywordList.indexOf(interaction) >= 0) return true;
        };
        var nextKey = _this.evaluate_criteria(evalfunc, interaction, loc.criteria);
        if (nextKey) {
            _this.go(loc.links[nextKey].target_id);
        }
    ...
    this.publish = {
        update_ui: function update_ui(data) {
            data.engine.events.publish("update_ui", data);
        },
        location_changed: function location_changed(data) {
            data.engine.events.publish("location_changed", data);
        }
    };
};
...
var EventHandlers = function EventHandlers() {
    ...
    this.add = function (ac) {
        ...
        if (this.items[ac.event_type] == undefined) {
            this.items[ac.event_type] = [];
        }
    ...
    this.execute = function (event_type, engine) {
        if (typeof event_type != "string") {
            throw new Error("ArgumentException: event_type. Expecting string. Actual value " + e);
        }
        if (typeof engine != "object") {
            throw new Error("ArgumentException: engine. Did you specify engine in data when firing event? Expecting object. Actual value " + listener);
        }

        // see if there is any action cotainer registered for the event
        if (this.items[event_type] == undefined) {
            return;
        }
        // execute
        this.items[event_type].forEach(function (ac) {
            ac.execute(engine);
        });
    };
};
...
var EventHandler = function EventHandler(event_type) {
...
var LocationCondition = function LocationCondition(location_id) {
    ...
    this.is_valid = function (engine) {
        return engine.player.location_id === this.location_id;
    };
};

// creates logical condition with injected validation logic
var CreateLogicalConditionClass = function CreateLogicalConditionClass(className, is_valid_func) {

    // using closure returns function with injected validation logic (is_valid_func)
    var LogicalConditionTemplate = function LogicalConditionTemplate(c1, c2) {
        if (typeof c1 != "object") {
            throw new Error("ArgumentException: c1. Expecting a Condition object. Actual value " + c1);
        }
        if (typeof c2 != "object") {
            throw new Error("ArgumentException: c1. Expecting a Condition object. Actual value " + c2);
        }
        // name of type. used for json serialization
        ...
        this.is_valid = function (engine) {
            return is_valid_func(this.c1, this.c2, engine);
        };
    };
...
// Class factory
var CreateActionClass = function CreateActionClass(className, execute_func) {

    var ActionTemplate = function ActionTemplate(args, repeat) {
        // name of type. used for json serialization
        this.className = className;
        // store arguments
        this.args = args;
...
var Location = function Location() {
    ...
    this.set_text = function (state, text) {
        this.texts[state] = text;
    };
    // links to other locations (array of links)
    this.links = [];
    // adds link to location
    this.add_link = function (l) {
        this.links.push(l);
    };
...
var LocationFactory = function LocationFactory() {
    // stores location
    this.l = null;
    // sets location instance
    this["new"] = function () {
        this.l = new Location();return this;
    };
    this["for"] = function (location) {
        this.l = location;return this;
    };
    this.id = function (location_id) {
        this.l.id = location_id;return this;
    };
    // sets state of location
    this.set_state = function (state) {
        this.l.state = state;return this;
    };
    // adds text to the location for given state
    this.set_text = function (state, text) {
        this.l.set_text(state, text);return this;
    };
    ...
    // add criteria
    this.set_criteria = function (criteria) {
        this.l.criteria = criteria;return this;
    };
    // Add Links = 
    this.set_links = function (links) {
        this.l.links = links;return this;
    };
...
var LocationRepository = function LocationRepository() {
    ...
    this.all = function () {
        list = [];for (var id in this.locations) {
...
var PubSub = function PubSub() {
    ...
    this.subscribe = function (e, listener) {
        if (typeof e != "string") {
            throw new Error("ArgumentException: e. Expecting string. Actual value " + e);
        }
        if (typeof listener != "function") {
            throw new Error("ArgumentException: listener. Expecting function. Actual value " + listener);
        }
